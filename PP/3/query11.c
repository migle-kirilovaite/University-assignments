...
int main(int argc, char *argv[]) {
	...
	if (db){							
        ...
        { 
                ...
                int philosophyDemo = 0; 
                int above10=0;
				for (int i = 0; i < s.load; ++i)// process each course taken by the student
                { 
                    if (!(strcmp(s.courses[i], "Philosophy")))
                    {
                        ++philosophyDemo; // counting
                    }
                    else if (s.grades[i]==10)
                    {
                        ++above10;
                    }

                }
				if (philosophyDemo >= 1 || above10>=1 ) // *** third filter, various other conditions
                {
